...
const MemoryLayerCard = ({ layer, index }) => (
  <Card
    delay={0.48 + index * 0.08}
    className="relative overflow-hidden p-5"
    style={{
      borderColor: layer.color || T.faint,
      background: layer.fill || 'rgba(255,255,255,0.72)'
    }}
  >
    <div
      className="absolute inset-x-0 top-0 h-1"
      style={{ background: layer.color || T.coralDark }}
    />
    <div className="flex items-start gap-4">
      <div className="rounded-[7px] border bg-white p-2.5" style={{ borderColor: layer.color || T.coralPale }}>
        <MiniIcon type={layer.icon || 'folder'} color={layer.color || T.coralDark} className="h-9 w-9" />
      </div>
      <div>
        <div className="text-[12px] uppercase" style={{ color: layer.color || T.coralDark, fontFamily: T.mono, letterSpacing: '0.18em', fontWeight: 850 }}>
          {layer.scope}
        </div>
        <div className="mt-1" style={{ color: T.ink, fontSize: 25, lineHeight: 1.05, fontWeight: 650 }}>
          {layer.title}
        </div>
        <div className="mt-2" style={{ color: T.muted, fontSize: 16.5, lineHeight: 1.28, fontWeight: 500 }}>
          {layer.body}
        </div>
      </div>
    </div>
...
const HarnessMemoryPerformanceVariant = ({ slide }) => {
  const memoryLayers = slide.memoryLayers || [];
  const reviewSteps = slide.reviewSteps || [];

  return (
    <Shell>
      <Marker slide={slide}>{slide.marker}</Marker>
      <JobSpine activeKey={slide.spineActive || 'context'} />

      <div className="absolute left-[4.8vw] right-[4.8vw] top-[5vh]" style={{ maxWidth: '86vw' }}>
        <h1 className="leading-[0.96]" style={{ fontSize: 'clamp(42px, 4.2vw, 68px)', fontWeight: 520 }}>
          <PartText parts={slide.parts} />
        </h1>
        {slide.subhead && (
          <motion.div {...fade(0.34)} className="mt-2 italic" style={{ color: T.muted, fontSize: 'clamp(18px, 1.35vw, 23px)', lineHeight: 1.22 }}>
            {slide.subhead}
          </motion.div>
        )}
      </div>

      <div className="absolute left-[4.8vw] right-[4.8vw] top-[20vh] bottom-[10vh] grid grid-cols-[48%_1fr] gap-6">
        <div className="flex min-h-0 flex-col gap-3.5">
          <Kicker>Scoped recall</Kicker>
          <div className="grid flex-1 grid-rows-3 gap-3.5">
            {memoryLayers.map((layer, index) => (
              <div key={`${layer.title}-${index}`} className="relative">
                {index > 0 && (
                  <motion.div
                    {...fade(0.62 + index * 0.08, 0)}
                    className="absolute -top-[15px] left-[37px] h-[15px] border-l-2"
                    style={{ borderColor: layer.color || T.coralPale }}
                  />
                )}
                <MemoryLayerCard layer={layer} index={index} />
              </div>
            ))}
          </div>
          {slide.governanceNote && (
            <motion.div {...fade(0.96)} className="rounded-[6px] border px-4 py-3" style={{ borderColor: T.coralPale, background: '#fff8f2', color: T.coralDark }}>
              <div className="flex items-center gap-3">
                <MiniIcon type="lock" color={T.coralDark} className="h-7 w-7" />
                <div style={{ fontSize: 17, lineHeight: 1.25, fontWeight: 620 }}>{slide.governanceNote}</div>
              </div>
            </motion.div>
          )}
        </div>

        <div className="grid min-h-0 grid-rows-[1fr_170px] gap-4">
          <Card delay={0.56} className="flex min-h-0 flex-col p-5" style={{ borderColor: T.blue, background: T.bluePale }}>
            <div className="flex items-center justify-between gap-4">
              <div>
                <Kicker style={{ color: T.blue }}>Application example</Kicker>
                <div className="mt-1.5" style={{ color: T.ink, fontSize: 34, lineHeight: 1.02, fontWeight: 680 }}>
                  {slide.useCaseTitle || 'Performance review assistant'}
                </div>
              </div>
              <div className="rounded-[8px] border bg-white p-3" style={{ borderColor: '#b9d3dc' }}>
                <MiniIcon type="observe" color={T.blue} className="h-12 w-12" />
              </div>
            </div>
            ...
            <div className="mt-4 grid flex-1 grid-rows-3 gap-3">
              {reviewSteps.map((step, index) => (
                <motion.div
                  ...
                  {...fade(0.78 + index * 0.1)}
                  className="grid grid-cols-[64px_1fr] items-center gap-4 rounded-[6px] border bg-white/82 px-4 py-3"
                  style={{ borderColor: index === 2 ? T.coral : '#b9d3dc' }}
                >
                  <div className="flex h-12 w-12 items-center justify-center rounded-[6px]" style={{ background: index === 2 ? '#fff1e9' : '#edf6f7', color: index === 2 ? T.coralDark : T.blue, fontFamily: T.mono, fontWeight: 900, letterSpacing: '0.08em' }}>
                    {String(index + 1).padStart(2, '0')}
                  </div>
                  <div>
                    <div className="text-[11px] uppercase" style={{ color: index === 2 ? T.coralDark : T.blue, fontFamily: T.mono, letterSpacing: '0.16em', fontWeight: 850 }}>
                      {step.kicker}
                    </div>
                    <div className="mt-1" style={{ color: T.ink, fontSize: 19.5, lineHeight: 1.22, fontWeight: 570 }}>
                      {step.body}
                    </div>
                  </div>
                </motion.div>
          ...
          <Card delay={0.92} className="overflow-hidden" style={{ borderColor: T.ink, background: T.ink }}>
            {slide.video?.src ? (
              <video
                src={slide.video.src}
              ...
              <div className="flex h-full items-center justify-between gap-6 px-6" style={{ color: '#fff' }}>
                <div>
                  <div className="text-[12px] uppercase" style={{ color: '#f3d1c4', fontFamily: T.mono, letterSpacing: '0.22em', fontWeight: 850 }}>
                    POC video ready
                  </div>
                  <div className="mt-2" style={{ fontSize: 28, lineHeight: 1.02, fontWeight: 650 }}>
                    {slide.video?.title || 'Performance management workflow'}
                  </div>
                  <div className="mt-2" style={{ color: 'rgba(255,255,255,0.72)', fontSize: 16.5, lineHeight: 1.25 }}>
                    {slide.video?.body || 'Add the demo recording and this panel will play it in place.'}
                  </div>
                </div>
                <div className="relative h-[96px] w-[158px] shrink-0 rounded-[6px] border" style={{ borderColor: 'rgba(255,255,255,0.32)', background: 'rgba(255,255,255,0.08)' }}>
                  <div className="absolute inset-3 rounded-[4px]" style={{ border: '1px solid rgba(255,255,255,0.22)' }} />
                  <div className="absolute left-1/2 top-1/2 h-0 w-0 -translate-x-[42%] -translate-y-1/2 border-y-[18px] border-l-[28px] border-y-transparent" style={{ borderLeftColor: '#fff' }} />
                </div>
              </div>
            )}
          </Card>
...
export const harnessVariantMap = {
  ...
  'harness-memory-performance': HarnessMemoryPerformanceVariant,
